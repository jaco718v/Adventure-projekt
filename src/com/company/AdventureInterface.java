...
public class AdventureInterface {
  ...
  public void help() {
    System.out.println("(go) + [Direction]\ttake [Item]\t\teat [Food]\t\t\tequip [Weapon]\n" +
        "search\t\t\t\tdrop [Item]\t\t\t\t\t\t\tinventory\n" +
  ...
  public void setInitialInventory(Item first, Item second, Item third) {
    ...
    broughtGear.add(first);
    broughtGear.add(second);
    broughtGear.add(third);
          ...
          switch (attackCase) {
            ...
            case EnemyPresent -> {
              ...
              while (engine.player.getCurrentRoom().getRoomEnemies().size() > 0 && !engine.player.isFleeFlag()) {
                ...
                System.out.println("Gabriels current health: " + engine.player.getHealth() + "/35");
                ...
                if (engine.enemyDeath()) {
                  System.out.println("You've defeated the enemy, it dropped a weapon!");
                }
